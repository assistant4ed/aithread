...
export default function ArticlesPage() {
    ...
    const PRESET_TOPICS = ["Hong Kong Tech", "Global Finance", "AI & Web3", "Local Property Market"];
    ...
    const handleFileUpload = async (articleId: string, file: File) => {
        ...
        try {
            ...
            await handleMediaSelect(articleId, data.url, data.type); // Changed 'image' to data.type
        } catch (e: any) {
    ...
    const handleGenerate = async () => {
        if (!generateTopic.trim()) {
            setGenerationError("Please enter or select a topic.");
            return;
        }
        ...
        try {
            const res = await fetch(`/api/workspaces/${workspaceId}/generate-article`, {
                method: "POST",
                headers: { "Content-Type": "application/json" },
        ...
        <div className="max-w-7xl mx-auto space-y-6 pb-20">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    {loading && <div className="animate-spin w-5 h-5 border-2 border-accent border-t-transparent rounded-full" />}
                </div>
                <button
                    onClick={() => setIsGenerateModalOpen(true)}
                    className="flex items-center gap-2 px-4 py-2 bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded-lg hover:bg-purple-500/20 transition-colors text-sm font-medium"
                >
            ...
            {/* AI Generate Modal */}
            {
                isGenerateModalOpen && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm animate-fade-in">
                        <div className="bg-surface border border-border rounded-xl shadow-2xl max-w-md w-full p-6 space-y-6" onClick={e => e.stopPropagation()}>
                            <div>
                                ...
                                <p className="text-sm text-muted mt-1">
                                    Uses live web search to find the latest news on your topic and drafts a new article instantly.
                                </p>
                            </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    <label className="text-xs font-semibold uppercase tracking-wider text-muted mb-2 block">Quick Topics</label>
                                    <div className="flex flex-wrap gap-2">
                                        {PRESET_TOPICS.map(topic => (
                                            <button
                                                key={topic}
                                                onClick={() => setGenerateTopic(topic)}
                                                className={`px-3 py-1.5 text-xs font-medium rounded-full border transition-colors ${generateTopic === topic
                                                    ? "bg-purple-500/20 border-purple-500/40 text-purple-300"
                                                    : "bg-surface border-border text-muted hover:text-foreground hover:bg-white/5"
                                                    }`}
                                            >
                                                {topic}
                                            </button>
                                ...
                                <div>
                                    <label className="text-xs font-semibold uppercase tracking-wider text-muted mb-2 block">Custom Topic</label>
                                    <input
                                        type="text"
                                        value={generateTopic}
                            ...
                            <div className="flex gap-3 pt-4 border-t border-border/50">
                                <button
                                    ...
                                    disabled={isGenerating}
                                    className="flex-1 px-4 py-2 border border-border rounded-lg text-sm font-medium hover:bg-white/5 transition-colors disabled:opacity-50"
                                >
                                    Cancel
                                </button>
                                <button
                                    ...
                                    className="flex-1 px-4 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-bold transition-colors disabled:opacity-50 flex items-center justify-center gap-2"
                                >
                                    {isGenerating ? (
                                        <>
